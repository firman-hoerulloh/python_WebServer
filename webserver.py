...
def run_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket: # membuat socket
        ...
        print(f'Mendengarkan permintaan pada port {PORT}...') #print koneksi yang di listen() pada port ...
...
def handle_request(client_socket):
    ...
    try:
        if file_name != '/': # jika filename bukan hanya "/" berarti client meminta file
            current_file_path = os.path.realpath(__file__) # mencari path alamat file python
            folder_path = os.path.dirname(current_file_path) # mencari folder path
            path = folder_path + "\\" + file_name[1:] #path file yang dicari
            f = open(path) #membuka file
            file_content = f.read() # membaca file
    except:
    ...
    if method == 'GET' and file_name != 'ERROR': # jika client merequest file
        ...
        response_headers = [ #html respone header
